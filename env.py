...
class MazeEnv(Env):
    def __init__(self, n_rooms: int, n_agents: int, random_state: Optional[int] = None):
        ...
        self.action_space = spaces.Discrete(4) # Need to change this to include KILL + FIX + SABOTAGE actions
    ...
    def step(self, action):
        ...
        return ((self.agent_positions, self.job_positions, self.completed_jobs, self.alive_agents), reward, done, truncated, {})
       ...
       # Draw agents and jobs 
        ...
        for i, agent in enumerate(self.agent_positions):
            ...
            if self.alive_agents[i] == 0:
                ...
                if i == self.turn:
                    ax.add_patch(patches.Rectangle(agent, 1, 1, color='green', alpha=0.3))
                else:
                    ax.add_patch(patches.Rectangle(agent, 1, 1, color='gray', alpha=0.3))
